...
if __name__ == "__main__":
    ...
    for input_foldername in args.input:
        filenames = [os.path.splitext(os.path.basename(filename))[0] for filename in glob.glob(os.path.join(input_foldername[0], "*.SBS"))]
        filenames += [os.path.splitext(os.path.basename(filename))[0] for filename in glob.glob(os.path.join(input_foldername[0], "*.sbs"))]
        filenames = list(set(filenames))  # Remove duplicates

        for filename in filenames:
            frame_manager = FrameManager(args.output, input_foldername, jpsxdec_path)
            pool.apply_async(frame_manager.get_raw_frames, args=(filename, "sbs",))
